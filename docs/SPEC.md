...
|----|------|
| 文档编号 | SPEC-0.2 |
| 状态 | **CR-applied（命名）** — RTL 端口冻结 `982ddd0a` |
| 前版冻结 | SPEC-0.1 **已冻结**（2026-09-03，人批准），对齐 RTL `32a7f5e0`。端口冻结 `1ed4d350` 已被 Lint ECO 取代为 `982ddd0a` |
...
| 对齐 RTL（端口冻结） | SHA `982ddd0a`（`982ddd0a54cf19dbeb39cf8f8f523c4e26e593f6`）；merge `581d9d34`（仅追溯）。旧端口冻结 `1ed4d350` 已失效 |
| 对齐 RTL（功能） | 宽度 / 协议 / 握手语义仍同 `32a7f5e0`；**不是**功能改写 |
| 接口名 | `{src}_{dst}_{meaning}`，datalink token 为 **`dll`**（不是 `dl`）。命名不变（CR-B 已落地） |

本文件是冻结门：芯片开发 PM 据此验收。SPEC-0.1 于 2026-09-03 人批准、对齐 RTL `32a7f5e0`。**CR-B 打破该冻结，仅限接口命名**；功能宽度、时序、fire 规则与协议不变。RTL 端口更名已落地：本文件产品接口表与 `rtl/` 均为 CR-B 名。**Lint ECO（PR62）打破端口冻结 `1ed4d350`**；当前端口冻结 SHA 为 `982ddd0a`（merge `581d9d34`，仅追溯）。命名 / 功能接口不变。F1（`ovf_l`）不变。

本修订不改写 FS-0.2.7 已实现功能。`982ddd0a` 是新的端口冻结 SHA（角色同旧 `1ed4d350` / 更早 `32a7f5e0`）。`32a7f5e0` 只标明功能宽度 / 协议 / 握手语义未改，不构成规格功能变更。
...
| 项 | 规则 |
|----|------|
| 当前状态 | **CR-applied（命名）** — SPEC-0.2。SPEC-0.1 冻结（2026-09-03 / RTL `32a7f5e0`）已被 CR-B 打破，**仅限命名**。Lint ECO 打破端口冻结 `1ed4d350`；当前冻结 `982ddd0a` |
...
| 功能改写 | 不得以“对齐 SHA `32a7f5e0` / `1ed4d350` / `982ddd0a`”或“CR-B 命名 / Lint ECO”为由改写 FS-0.2.7 已锁定行为 |
| 空洞升级 | §非目标项若要进入冻结正文，须变更请求，并更新本文件版本号 |
| RTL 更名 | 已落地（CR-B）。端口冻结已被 Lint ECO 更新为 `982ddd0a`（`982ddd0a54cf19dbeb39cf8f8f523c4e26e593f6`）；merge `581d9d34`。旧 `1ed4d350` 对端口已失效 |
...
- RTL SHA `982ddd0a`（端口冻结；PR62 merge `581d9d34`）。功能宽度 / 协议 / 握手语义仍同 `32a7f5e0`。F1 不变。
