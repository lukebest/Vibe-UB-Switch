...
| `origin/main` HEAD | `473df7a4` (PR46) |
...
| 2 | **`ovf_l` CDC-WARN.** `rtl/port/vibe_port.sv:244`: 1-cycle `rxclk` sticky/pulse OR-reduction of `ovf_l`, then 2-FF into `clk_fab` as `afifo_ovf`. No pulse stretcher / req-ack. A single-cycle overflow can be missed (1.25 GHz ↔ 922 MHz). Data path is AFIFO-protected. Pre-existing at freeze; **not an ECO**; **not a fake signoff gap**. | Design + 芯片开发PM | **OPEN** — permanent frozen waiver under **F1**. Nightly [`reports/cdc/2026-09-08.md`](../reports/cdc/2026-09-08.md) (unchanged vs 2026-09-07 / 2026-09-04 baseline). Record: [`reports/signoff/DECISION-F-2026-09-07.md`](../reports/signoff/DECISION-F-2026-09-07.md). |
| 3 | **xbar LATCH × 2 + fabric UNOPTFLAT × 2.** Verilator `-Wall`: `vibe_xbar.sv` combo `req`/`win` not assigned on every path; `vibe_fabric` ready/valid combo loops (`x_in_v`, `xb_r`). Counts unchanged on 2026-09-08 (0 new keys). Waived in [`reports/lint/WAIVERS.md`](../reports/lint/WAIVERS.md). Pre-existing; RTL frozen. | Design | **OPEN** — known lint debt. Not an ECO under freeze. |
| 4 | **Missing STATUS / RISKS** (PM cannot see gates / debts in-repo). | Docs / 芯片开发PM | **CLOSED** — files exist on main (PR31 `a3c8a331`). |
| 5 | **CR-B iface rename (Option B) approved, not landed.** Full `{src}_{dst}_{meaning}` rename (`dll` not `dl`) will break freeze SHA `32a7f5e0` and void 3/3 greens when Design/Verification land. Until then, RTL names stay as frozen; do not treat this CR as rename-complete or as a signoff hole. | Xia + Design + Verification + 芯片开发PM | **OPEN** — CR approved 2026-09-08; rename **in progress**. Record: [`docs/cr/CR-IFACE-RENAME-B-2026-09-08.md`](cr/CR-IFACE-RENAME-B-2026-09-08.md). Inventory: [`docs/cr/CR-IFACE-NAMING-DRAFT-2026-09-08.md`](cr/CR-IFACE-NAMING-DRAFT-2026-09-08.md). |
...
| **F** | **`ovf_l` disposition.** **DECIDED 2026-09-07 (Asia/Shanghai): F1.** Permanently record `ovf_l` CDC-WARN at `rtl/port/vibe_port.sv:244` as frozen WARN/waiver. Keep RTL freeze SHA `32a7f5e0`. No ECO. No issue. Not a signoff hole. | Not blocked — Luke chose **F1**. Nightly CDC-WARN unchanged vs 2026-09-04 baseline. Record: [`reports/signoff/DECISION-F-2026-09-07.md`](../reports/signoff/DECISION-F-2026-09-07.md). | **Decided: F1.** Permanent frozen WARN/waiver. Leave RTL as-is. Do not ECO. Do not open an issue. Do not call it a signoff hole. |
| **G** | **Interface naming.** **DECIDED 2026-09-08 (Asia/Shanghai): Option B.** Formal CR + full RTL/TB/SPEC/AS rename to `{src}_{dst}_{meaning}` (`dll` not `dl`; handshake `_vld`/`_ready`; clocks/resets exempt). No functional width/protocol change. F1 (`ovf_l`) still stands — no ECO. | Not blocked — Luke chose **B**. Inventory: [`docs/cr/CR-IFACE-NAMING-DRAFT-2026-09-08.md`](cr/CR-IFACE-NAMING-DRAFT-2026-09-08.md). Record: [`docs/cr/CR-IFACE-RENAME-B-2026-09-08.md`](cr/CR-IFACE-RENAME-B-2026-09-08.md). Rename **not** implemented on this snapshot. | **Decided: B.** Pending Xia (SPEC/AS/CHANGELOG), Design (`rtl/`), Verification (`tb/` + re-gate), PM acceptance. Do not claim rename done. Path B still **NOT PASS**. New 3/3 series only after reports/regress land against the new SHA. |
...
- Nightly [`reports/regress/2026-09-08.md`](../reports/regress/2026-09-08.md) is post-gate health, **not** consecutive-green 4/3.
- CR-B Option B is **approved**, not landed. This file does not claim the iface rename is done, does not break freeze SHA `32a7f5e0` by itself, and does not start a new 3/3 series.
