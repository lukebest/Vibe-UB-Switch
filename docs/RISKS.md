...
| `origin/main` HEAD | `63c41a17` (PR52 merge) |
...
| 5 | **CR-B TB lag.** Design **landed** at freeze `1ed4d350` (`rtl/` only; ports `{src}_{dst}_{meaning}`). PR51 `777865f0` is merge-trace only. `tb/vibe` is not yet rewritten against DUT `1ed4d350`. Old consecutive greens **3/3** (2026-09-03 run1/2/3 vs `32a7f5e0`) are **VOID**. Nightly health may fail until the TB rename lands — **expected**; not a consecutive green; not an ECO chase. Chat PASS does **not** count. New series starts at **1/3** only after `reports/regress/` lands. | Verification + 芯片开发PM | **OPEN** — active. SPEC-0.2 naming frozen (PR50). Xia CHANGELOG/SPEC pin **done** (PR52, freeze `1ed4d350`). Record: [`docs/cr/CR-IFACE-RENAME-B-2026-09-08.md`](cr/CR-IFACE-RENAME-B-2026-09-08.md). |
...
| **G** | **Interface naming.** **DECIDED 2026-09-08 (Asia/Shanghai): Option B.** Formal CR + full rename to `{src}_{dst}_{meaning}` (`dll` not `dl`). SPEC-0.2 naming **frozen** (PR50). Design **landed** (freeze `1ed4d350`; PR51 merge `777865f0` is trace only). Xia CHANGELOG/SPEC pin **done** (PR52). Verification **not** landed — TB rewrite + new 0/3→1/3 series pending. No functional width/protocol change. F1 (`ovf_l`) still stands — no ECO. | Not blocked on the decision. **Blocked on Verification.** Inventory: [`docs/cr/CR-IFACE-NAMING-DRAFT-2026-09-08.md`](cr/CR-IFACE-NAMING-DRAFT-2026-09-08.md). Record: [`docs/cr/CR-IFACE-RENAME-B-2026-09-08.md`](cr/CR-IFACE-RENAME-B-2026-09-08.md). | **Decided: B.** Design + Xia pin done. Verification: rewrite `tb/vibe` vs DUT `1ed4d350`; 1/3 only after `reports/regress/` lands; chat PASS does not count. Path B still **NOT PASS**. |
...
- No coverage / LINE / TP numbers for DUT `1ed4d350` (none committed).
- Old consecutive greens **3/3** (2026-09-03 vs `32a7f5e0`) are **VOID**. Do not count them. Do not count chat PASS. Do not count nightly health as 4/3 or as the new 1/3.
- Nightly health may fail until TB rename lands — expected; not an ECO chase.
- CR-B Option B **RTL has landed** at freeze `1ed4d350`. This file does **not** claim TB rename done, does **not** claim a new 3/3, and does **not** claim signoff.
