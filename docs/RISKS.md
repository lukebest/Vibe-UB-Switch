...
| `origin/main` HEAD | `67d7d4d5` (PR30) |
| RTL freeze SHA | `32a7f5e0` — `rtl/` unchanged vs this SHA |
| Companion | [`docs/STATUS.md`](STATUS.md) |

Facts only. **No WNS, TNS, or slack numbers** — OpenSTA was not run; [`reports/signoff/`](../reports/signoff/) is `.gitkeep` only; [`reports/synth/timing_summary.rpt`](../reports/synth/timing_summary.rpt) is `STATUS: not_run`.
...
| 4 | **Missing STATUS / RISKS** (PM cannot see gates / debts in-repo). | Docs / 芯片开发PM | **CLOSING** — this PR adds [`docs/STATUS.md`](STATUS.md) and this file. |
...
|----|----------|-------------------|------------------------|
| **E** | **Signoff next-gate definition.** What must exist for impl gate → PASS (mapped top? which PDK? STA tool? DRC/LVS?) | [`reports/signoff/`](../reports/signoff/) empty; methodology QoR is explicitly **not** tapeout; SPEC §16 says QoR is **not** a SPEC must | Impl gate stays **NOT PASS**. Do not invent WNS/TNS. Do not treat Sky130 1.25 GHz fail as a missing file. |
| **F** | **`ovf_l` disposition.** Permanent waiver vs later ECO (pulse stretch / req-ack). | CDC-WARN recorded; RTL freeze SHA `32a7f5e0`; no issue opened | Leave RTL as-is. Do not ECO. Do not call it a signoff hole. |
