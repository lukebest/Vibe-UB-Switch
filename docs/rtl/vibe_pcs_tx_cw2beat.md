...
PCS streaming helper used by `vibe_pcs_tx` `u_cw`) under the
pycircuit → rtl flow.
...
| `cw_data[1023:0]` | in | FEC codeword (hi then lo) |
| `cw_vld` | in | 1024b codeword available |
| `cw_ready` | out | combo `!have_hi && !have_lo` |
| `beat_data[511:0]` | out | combo `have_hi ? hi : lo` |
| `beat_vld` | out | combo `have_hi \|\| have_lo` |
...
One always block plus combo ready/valid. Accept parks
`cw_data[1023:512]` in `hi` and `cw_data[511:0]` in `lo` and sets
both `have_*`. The first handshake emits `hi` and clears `have_hi`.
The second emits `lo` and clears `have_lo`. Then `cw_ready` is high
again. Accept and beat handshake are mutually exclusive on the
combo flags.
