...
| Status | **Draft only** — not approved, not implemented |
| Date | 2026-09-08 |
...
`git diff 32a7f5e0 -- rtl/` must stay empty until an approved Option B CR lands.
...
4. Approval of Option A vs B is a human decision (Luke / 芯片开发PM). Default until decided: **do nothing to RTL**.
...
| Choice | Effect |
|--------|--------|
| **A** | Waves use `nw_dll_*` labels; DUT names stay; freeze intact |
| **B** | Formal CR + RTL/TB/SPEC rename + re-verification + void 3/3 |
| **Neither** | Draft remains on file; current inconsistent names stay |
