...
| `origin/main` HEAD | `67d7d4d5` — Merge PR30 (nightly lint+CDC 2026-09-04) |
| RTL freeze SHA | `32a7f5e0` (`32a7f5e0c3f04762aa27dae73b000e55773195da`) |
| `git diff 32a7f5e0 -- rtl/` | **empty** — no `rtl/` functional diff vs freeze SHA |
...
| Impl signoff | **NOT PASS** | [`reports/signoff/`](../reports/signoff/) contains only `.gitkeep`. Methodology QoR: [`reports/synth/2026-09-03.md`](../reports/synth/2026-09-03.md) — **no top netlist**, **no WNS/TNS**. |
...
| Signoff package | SPEC §16: QoR is **not** a SPEC must | N/A (no RTL ECO for signoff) | N/A | **NOT PASS** — `reports/signoff/` keepers only; OpenSTA **not run**; no `sta_wns_tns.rpt` | FPGA **deferred** |
...
| Item | Count | As of |
|------|------:|--------|
| Open GitHub issues | **0** | 2026-09-04, `lukebest/Vibe-UB-Switch` |
| Open GitHub PRs | **0** | same (this STATUS/RISKS PR is the next one) |
...
1. **STATUS / RISKS land** — this pair of files. Close the “missing project status” hole.
2. **Signoff PLAN** — human **decision E**: define the next impl gate (what “signoff PASS” means without inventing Sky130 WNS). Until then the impl gate stays **NOT PASS**.
3. **Freeze watch** — keep `git diff 32a7f5e0 -- rtl/` empty. Interface / function change after SPEC-0.1 **已冻结** needs a change request. Watch `ovf_l` (decision **F**) without treating it as an ECO.
