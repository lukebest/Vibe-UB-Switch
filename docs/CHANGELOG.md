...
- Next leaf `vibe_xbar` is described in
  `pycircuit/fabric/vibe_xbar.py` and landed
  hand-finished at `rtl/fabric/vibe_xbar.sv`.
  **Same ports / 4-port xbar behavior** as tip
  `782f2181` / freeze `302ac943` (`clk`, `rst_n`,
  `status_up[3:0]`, `in_data[511:0][0:3]`,
  `in_vld[3:0]`, `in_sop[3:0]`, `in_eop[3:0]`,
  `in_dst[1:0][0:3]`, `in_ready[3:0]`,
  `out_data[511:0][0:3]`, `out_vld[3:0]`,
  `out_sop[3:0]`, `out_eop[3:0]`, `out_ready[3:0]`;
  async-low `or negedge rst_n`; unpacked arrays;
  candidate grant independent of `out_ready`;
  accept (`out_vld` / `in_ready`) needs `out_ready`;
  down ports get no data DLLDP; ingress RR on
  conflict from `rr[e]`; one full packet per grant
  (`lock` until EOP); `!rst_n` clears `lock` /
  `locked` / `rr`; EOP advances `rr <= lock+1`;
  full combo defaults on `req` / `win` (LATCH ECO);
  AS-0.1 §8). Self-contained (no child instances).
  Seventh fabric leaf after `vibe_fecn_mark` /
  `vibe_vl_rr` / `vibe_route_lu` / `vibe_port_sel` /
  `vibe_voq_egr` / `vibe_saf_ing`. Instantiated by
  `vibe_fabric` (`u_xbar`). Ports / xbar match stock
  (header-only vs stock). Official `.vlt` not
  expanded. Not a chip rewrite. No SPEC CR. F1
  `ovf_l` untouched. Stage-1 `vibe_afifo`, stage-2
  `vibe_pma_bnd`, stage-3 `vibe_sync2`, stage-4
  ...
  stage-28 `vibe_vl_rr`, stage-29 `vibe_route_lu`,
  stage-30 `vibe_port_sel`, stage-31 `vibe_voq_egr`,
  and stage-32 `vibe_saf_ing` left intact. PCS tx /
  rx tops, remaining DLL wraps (`vibe_dll_tx`, dll
  top), and `vibe_fabric` top not in this leaf.
- Regenerator: `make -C pycircuit vibe_xbar`. Regime remains
  **UNFROZEN** (Path B hold). Do not treat this leaf as a new freeze pin.
...
- Next leaf `vibe_voq_egr` is described in
  `pycircuit/fabric/vibe_voq_egr.py` and landed
  hand-finished at `rtl/fabric/vibe_voq_egr.sv`.
  **Same ports / VOQ + 1 µs deadlock behavior** as tip
  `8ec96a2a` / freeze `302ac943` (`clk`, `rst_n`,
  `wr_vl[3:0]`, `wr_en`, `wr_data[511:0]`, `wr_sop`,
  `wr_eop`, `wr_ready`, `rd_vl[3:0]`, `rd_en` /
  `rd_data[511:0]`, `rd_sop`, `rd_eop`,
  `nonempty[15:0]`, `occ_vl0[5:0]`, `deadlock_drop`,
  `deadlock_cnt[31:0]`; parameter `DEPTH=32`; async-low
  `or negedge rst_n`; 16 VL × DEPTH mem + sop/eop/age;
  `wr_ready` when `occ < DEPTH`; `rd_*` combo from
  `mem[rd_vl][rptr]`; `occ_vl0 = wptr[0]-rptr[0]`;
  `nonempty[v]=(wptr!=rptr)`; `!rst_n` clears
  `deadlock_*` and `wptr`/`rptr`; enqueue
  `age=VIBE_US_CYC`; age countdown; nonempty and
  head age 0 → auto-advance `rptr` + pulse
  `deadlock_drop` + inc `deadlock_cnt`; AS-0.1
  §8/§14). Self-contained (no child instances).
  Fifth fabric leaf after `vibe_fecn_mark` /
  `vibe_vl_rr` / `vibe_route_lu` / `vibe_port_sel`.
  Instantiated by `vibe_fabric` (`g_egr.u_voq`).
  Ports / VOQ match stock (header-only vs stock).
  Official `.vlt` not expanded. Not a chip rewrite. No
  SPEC CR. F1 `ovf_l` untouched. Stage-1 `vibe_afifo`,
  stage-2 `vibe_pma_bnd`, stage-3 `vibe_sync2`, stage-4
  `vibe_rst_sync`, stage-5 `vibe_gear_128_160`,
  ...
  `vibe_dll_retry_req_sm`, stage-27 `vibe_fecn_mark`,
  stage-28 `vibe_vl_rr`, stage-29 `vibe_route_lu`,
  and stage-30 `vibe_port_sel` left intact. PCS tx /
  rx tops, remaining DLL wraps (`vibe_dll_tx`, dll
  top), and `vibe_fabric` top not in this leaf.
  `vibe_saf_ing` / `vibe_xbar` not migrated here.
- Regenerator: `make -C pycircuit vibe_voq_egr`. Regime remains
  **UNFROZEN** (Path B hold). Do not treat this leaf as a new freeze pin.

## 2026-09-23 (Decision I stage-30 — vibe_port_sel)

### Changed

- Next leaf `vibe_port_sel` is described in
  ...
  **Same ports / port-select behavior** as tip
  ...
  `drop`, `drop_down_cnt[31:0]`; async-low
  `or negedge rst_n`; `available = bitmap & status_up`
  forced 0 if `drop_g1`; empty after filter → Default;
  Default all-0 → port 0 bitmap `4'b0001`; AND
  `status_up`; still empty → drop + increment
  `drop_down_cnt`, no flood; RT=00 per-flow sticky RR
  (`fidx=vl` compact, sticky `[0:15]`); RT=01 else
  per-packet RR via `rr`; `pick_rr(bm, start)` walks 4
  ports from start; AS-0.1 §2/§8). Self-contained (no
  child instances). Fourth fabric leaf after
  `vibe_fecn_mark` / `vibe_vl_rr` / `vibe_route_lu`.
  Instantiated by `vibe_fabric` (`u_ps`, `g_rt.u_psi`).
  Ports / select match stock (header-only vs stock).
  Official `.vlt` not expanded. Not a chip rewrite. No
  SPEC CR. F1 `ovf_l` untouched. Stage-1 `vibe_afifo`,
  stage-2 `vibe_pma_bnd`, stage-3 `vibe_sync2`, stage-4
  `vibe_rst_sync`, stage-5 `vibe_gear_128_160`,
  ...
  `vibe_dll_retry_buf`, stage-26
  `vibe_dll_retry_req_sm`, stage-27 `vibe_fecn_mark`,
  stage-28 `vibe_vl_rr`, and stage-29 `vibe_route_lu`
  left intact. PCS tx / rx tops, remaining DLL wraps
  (`vibe_dll_tx`, dll top), and `vibe_fabric` top not
  in this leaf. `vibe_voq_egr` / `vibe_saf_ing` /
  `vibe_xbar` not migrated here.
- Regenerator: `make -C pycircuit vibe_port_sel`. Regime remains
  **UNFROZEN** (Path B hold). Do not treat this leaf as a new freeze pin.

## 2026-09-23 (Decision I stage-29 — vibe_route_lu)

### Changed

- Next leaf `vibe_route_lu` is described in
  ...
  **Same ports / CFG0_ROUTE_TABLE lookup** as tip
  ...
  `or negedge rst_n`; `!rst_n || device_rst` clears
  table + outputs; on `wr_en` `tbl[wr_idx[7:0]] <=
  wr_data`; on `lu_vld` RT=10/11 pulse `drop_g1` and
  `bitmap=0`, else `bitmap <= tbl[dest[7:0]][3:0]`;
  no Dijkstra, no treat-as-RT=00, no RT rewrite;
  AS-0.1 §2/§8 + FS-0.2.3 G1). Self-contained (no
  child instances). Third fabric leaf after
  `vibe_fecn_mark` / `vibe_vl_rr`. Instantiated by
  `vibe_fabric` (`u_rt`, `g_rt.u_rti`). Ports / lookup
  ...
  `vibe_dll_retry_buf`, stage-26
  `vibe_dll_retry_req_sm`, stage-27 `vibe_fecn_mark`,
  and stage-28 `vibe_vl_rr` left intact. PCS tx / rx
  tops, remaining DLL wraps (`vibe_dll_tx`, dll top),
  and `vibe_fabric` top not in this leaf.
- Regenerator: `make -C pycircuit vibe_route_lu`. Regime remains
  **UNFROZEN** (Path B hold). Do not treat this leaf as a new freeze pin.
