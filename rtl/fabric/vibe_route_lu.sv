// AS-0.1 §2/§8: CFG0_ROUTE_TABLE dest → 4-bit egress bitmap.
// G1: RT=10 and RT=11 DROP, increment rt_shortest_unimpl, irq. No Dijkstra, no RT=00, no rewrite.
    ...
    if (!rst_n || device_rst) begin
      ...
      if (lu_vld) begin
        if (rt == 2'b10 || rt == 2'b11) begin
          ...
          drop_g1            <= 1'b1;
          bitmap             <= 4'd0;
          rt_shortest_unimpl <= rt_shortest_unimpl + 32'd1;
        end else begin
