...
module vibe_pcs_rx (
  ...
  vibe_pcs_rx_amctl_lock u_l0 (.clk(clk), .rst_n(rst_n), .in_vld(afifo_pcs_lane_vld), .in_data(afifo_pcs_lane0),
    .locked(am_locked[0]), .lid(lid0), .lid_bad(bad0), .is_amctl(am0), .sdf(sdf0), .edf(edf0));
  vibe_pcs_rx_amctl_lock u_l1 (.clk(clk), .rst_n(rst_n), .in_vld(afifo_pcs_lane_vld), .in_data(afifo_pcs_lane1),
    .locked(am_locked[1]), .lid(lid1), .lid_bad(bad1), .is_amctl(am1), .sdf(), .edf(edf1));
  vibe_pcs_rx_amctl_lock u_l2 (.clk(clk), .rst_n(rst_n), .in_vld(afifo_pcs_lane_vld), .in_data(afifo_pcs_lane2),
    .locked(am_locked[2]), .lid(lid2), .lid_bad(bad2), .is_amctl(am2), .sdf(), .edf(edf2));
  vibe_pcs_rx_amctl_lock u_l3 (.clk(clk), .rst_n(rst_n), .in_vld(afifo_pcs_lane_vld), .in_data(afifo_pcs_lane3),
  ...
  vibe_pcs_scramble u_d0 (.clk(clk), .rst_n(rst_n),
    ...
    .en(afifo_pcs_lane_vld && !am0), .in_vld(afifo_pcs_lane_vld), .in_data(afifo_pcs_lane0), .out_vld(dv), .out_data(d0));
  vibe_pcs_scramble u_d1 (.clk(clk), .rst_n(rst_n),
    .lane_id(2'd1), .seed_load(sl1),
    .en(afifo_pcs_lane_vld && !am1), .in_vld(afifo_pcs_lane_vld), .in_data(afifo_pcs_lane1), .out_vld(), .out_data(d1));
  vibe_pcs_scramble u_d2 (.clk(clk), .rst_n(rst_n),
    .lane_id(2'd2), .seed_load(sl2),
    .en(afifo_pcs_lane_vld && !am2), .in_vld(afifo_pcs_lane_vld), .in_data(afifo_pcs_lane2), .out_vld(), .out_data(d2));
  vibe_pcs_scramble u_d3 (.clk(clk), .rst_n(rst_n),
    .lane_id(2'd3), .seed_load(sl3),
    .en(afifo_pcs_lane_vld && !am3), .in_vld(afifo_pcs_lane_vld), .in_data(afifo_pcs_lane3), .out_vld(), .out_data(d3));
  ...
  assign wr = pcs_dll_ready && !pcs_dll_vld && !pend_vld;
  always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      rem      <= 320'd0;
      remv     <= 1'b0;
      ...
      if (pcs_dll_vld && pcs_dll_ready)
        pcs_dll_vld <= 1'b0;

      // AMCTL is skipped before unpack. Do not carry rem across the marker.
      // A syndrome-0 non-null window must raise pcs_dll_vld even during hunt.
      if (am_skip) begin
        rem      <= 320'd0;
        remv     <= 1'b0;
        pend     <= 640'd0;
      ...
      if (pend_vld && !pcs_dll_vld && pcs_dll_ready && !am_skip) begin
        if (!flit_null(pend[639:480])) begin
          pcs_dll_data <= pend;
          pcs_dll_vld  <= 1'b1;
        end
        pend_vld <= 1'b0;
      end else if (wv && wr && !pcs_dll_vld && !am_skip) begin
        if (win_null) begin
          rem      <= 320'd0;
          remv     <= 1'b0;
