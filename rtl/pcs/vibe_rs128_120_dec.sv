  ...
  always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      if (start) begin
        ...
        s0 <= s0 ^ in_sym;
        s1 <= gf_mul2(s1) ^ in_sym;
        s2 <= vibe_gf256_mul(s2, 8'd4) ^ in_sym;
        s3 <= vibe_gf256_mul(s3, 8'd8) ^ in_sym;
        s4 <= vibe_gf256_mul(s4, 8'd16) ^ in_sym;
        s5 <= vibe_gf256_mul(s5, 8'd32) ^ in_sym;
        s6 <= vibe_gf256_mul(s6, 8'd64) ^ in_sym;
        s7 <= vibe_gf256_mul(s7, 8'd128) ^ in_sym;
        if (cnt < 8'd120)
          msg[cnt] <= in_sym;
        if (cnt == 8'd127) begin
          busy     <= 1'b0;
          done     <= 1'b1;
          fec_fail <= |(s0 ^ in_sym) | |s1 | |s2 | |s3 | |s4 | |s5 | |s6 | |s7;
