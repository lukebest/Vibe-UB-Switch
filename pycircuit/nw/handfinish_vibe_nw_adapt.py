"""Emit the product SystemVerilog for vibe_nw_adapt (hand-finished).

Keeps tip ports, unused ``clk`` / ``rst_n`` pins, and the combo
512b vld/ready mux (AS-0.1: LinkReady in ready; mgmt inject
priority over VOQ). pycc netlists are a prototype only; this
file is what lands in ``rtl/nw/vibe_nw_adapt.sv``.
"""
...
// Regenerate: make -C pycircuit vibe_nw_adapt
"""
...
def main() -> int:
    ...
    dest = repo / "rtl" / "nw" / "vibe_nw_adapt.sv"
    write_rtl(dest)
    print(f"wrote {dest}")
    return 0
