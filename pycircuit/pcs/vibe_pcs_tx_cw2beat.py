...
combo ``cw_ready`` / ``beat_vld`` / ``beat_data``, and a single
always-block split. Landed SV is hand-finished to keep those freeze
semantics. Leave ``vibe_pcs_tx`` / amctl / pack / FEC / RS / rx for
later stages.
"""
...
def build(m: Circuit) -> None:
    """Split one 1024b codeword into two ready/valid 512b beats.
    ...
    ``have_hi ? hi : lo``. Accept loads both halves; consume
    drops ``have_hi`` first, then ``have_lo``. Accept and consume
    are mutually exclusive in stock (ready and valid never both).
    """
    clk = m.clock("clk")
    rst = m.reset("rst")
    ...
    consume = beat_vld & beat_ready

    # Accept first; consume last so NBA last-wins matches stock
    # (they do not overlap: cw_ready and beat_vld are exclusive).
    ...
    have_hi.set(u(1, 0), when=consume & have_hi.out())
    have_lo.set(u(1, 0), when=consume & ~have_hi.out())
