"""Emit the product SystemVerilog for vibe_dll_rx (hand-finished).

Keeps tip ports, async-low ``rst_n``, ``include "vibe_ub_fn.vh"``
for ``vibe_lph_cfg`` / ``vibe_lph_vl`` / ``vibe_pkt_bytes``,
parameter ``RXBUF`` default 1024, 640b PCS → 4 flits / unBCRC /
512b NW remainder, LPH first 160b, EOP drop leftover, CFG0
terminate, FEC/BCRC fail → Go-Back-N, and ``start_ack`` tied 0.
pycc netlists are a prototype only; this file is what lands in
``rtl/dll/vibe_dll_rx.sv``.
"""
...
// Regenerate: make -C pycircuit vibe_dll_rx
"""
...
module vibe_dll_rx #(
  ...
  wire        can_emit  = (emit_n != 8'd0) && (!dll_nw_vld || dll_nw_ready);
  ...
  always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      if (!link_up && (have || (by_n != 8'd0))) begin
        dll_nw_data <= {by_lj[1279:800], 1'b0, 1'b1, 30'd0};
        dll_nw_vld  <= 1'b1;
      end
      by_lj    <= 1280'd0;
      by_n     <= 8'd0;
      ...
      cfg0_hit  <= 1'b0;
      bcrc_fail <= 1'b0;
      if (dll_nw_vld && dll_nw_ready)
        dll_nw_vld <= 1'b0;

      if (pcs_dll_vld && pcs_dll_ready) begin
        if (is_cfg0) begin
          cfg0_hit  <= 1'b1;
          cfg0_data <= pcs_dll_data;
      ...
      if (have && (by_n <= 8'd80)) begin
        ...
        dll_nw_data <= by_lj[1279:768] & ({512{1'b1}} << (16'd512 - emit_n * 16'd8));
        dll_nw_vld  <= 1'b1;
...
def main() -> int:
    ...
    dest = repo / "rtl" / "dll" / "vibe_dll_rx.sv"
    write_rtl(dest)
    print(f"wrote {dest}")
    return 0
