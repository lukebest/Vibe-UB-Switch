...
module tc_cfg9_no_icrc;
  ...
  initial begin
    ...
    nw_fab_data[0] = vibe_tb_mk_beat(in_f);
    nw_fab_vld[0] = 1;
    @(posedge clk);
    @(negedge clk);
    ...
    if (saf_f === 160'd0)
      saf_f = u_fab.saf_d[0][511:352];
    if (fab_mgmt_cfg6_hit[0]) begin
      $display("FAIL tc_cfg9_no_icrc");
      $display("  stimulus : CFG9 RT=00 dest=1 (not terminate class)");
      $display("  expected : fab_mgmt_cfg6_hit=0 (CFG9 has no ICRC; forward)");
      $display("  actual   : fab_mgmt_cfg6_hit=1");
      $display("  hier     : u_fab.fab_mgmt_cfg6_hit");
      ...
      $display("  hier     : u_fab.x_in_v / fab_mgmt_cfg6_hit");
      $display("  reproduce: make -C tb/vibe units");
      fail = 1;
    end
