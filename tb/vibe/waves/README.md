...
**`make waves` lives on the TB branch `cursor/vibe-tb-g1-6065` (PR #9).**
It is not on `main` / RTL-only checkouts. If you see
`No rule to make target 'waves'`, you are not on that TB branch.
...
Branch must be **`cursor/vibe-tb-g1-6065` (PR #9)**. Overlay PR8 RTL for sim only; never commit `rtl/`.

```bash
git fetch origin
git checkout cursor/vibe-tb-g1-6065
