...
| `tc_nw_pkt_pma_loopback` | TP-PHY-012: TX→PMA→RX inverse, NW packet on `fab_rx` | **FAIL** after retest vs PR4 RTL `7319da84` (sim-only, not on this TB branch). Same expected LPH (CFG=3 RT=00 SCNA=A11A DCNA=B22B). Checker not relaxed. **Not patched.** |

DUT cannot be put in FEC bypass without an `rtl/` edit (`assign fec_mode = VIBE_FEC_T4`). Golden uses T=4.

`tc_nw_pkt_to_pma_tx` **PASS** (Icarus) vs `7319da84`: DLL LPH + 260 PMA pack + 104 PCS-lane golden + 260 gear golden (lane golden count moved from 106 with the 960+8 FEC change; score not relaxed).
...
Retest vs PR4 RTL `7319da84142e3631e4581bcada6ad0b2f90f6731` (`cursor/as01-rtl-82c7` HEAD; 960+8 FEC / fifth 512-beat / inverse G1 both 640s; not committed on this TB branch): still **FAIL**. Expected LPH unchanged. `fab_rx_vld` did rise, but flit0 was not CFG=3/RT=00/SCNA=A11A/DCNA=B22B.

| Probe | Peak (during 20000 `clk_fab`) | End |
|---|---|---|
| `fab_rx_vld` | 1 (`saw_fab_rx=1`) | 0 |
| expected LPH | 0 (never) | flit0=`730cda37b134e06162d6714fd271171c023fc635` (CFG=6 RT=00 SCNA=171C DCNA=D271) |
